...
| `AA_DATAFAST_SITE_ID` | no | unset | both | no | DataFast analytics site id (`dfid_...`). **Public, not a secret** — it is rendered into every visitor's HTML. Unset or empty means analytics is off: no script tag is rendered and the app-origin CSP keeps `script-src 'self'` / `connect-src 'self'` with no third-party host. The reported domain is derived from `BASE_URL`. Never applied to the sandboxed artifact frame. |
| `AA_HERO_ARTIFACT_PATH` | no | `/a/KbLJ0zvyiGadXLHUs2E5Rb` | both | no | Share path of the artifact the cloud marketing homepage links to as "Live artifact" and polls for its published state. An artifact id belongs to one database, so the default only resolves on the instance it was seeded on. Point it at your own share, or set it **empty** to declare that this host has no hero artifact: the homepage link disappears and the background poller never starts. Empty is a value, not an absence — it does not fall back to the default. Must start with `/`. |
