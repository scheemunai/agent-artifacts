...
- `public/assets/build-missing.css` — hand-written fallback stylesheet served when the CSS build has not run
- `public/assets/dashboard-m4.js`
- `public/assets/og-fallback.png` — rendered by the first-party Open Graph pipeline (`src/lib/og.ts`), regenerated by `pnpm run build:og-fallback`
